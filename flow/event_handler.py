...
class FileEventHandler(object):
  ...
  def task_specs(self) -> List[TaskSpec]:
    ...
    downloaded = [io.download(path) for path in paths]
    specs = [TaskParser(path).to_spec() for path in downloaded]
  ...
  def _handle_new_input(self, src_path: str) -> None:
    logging.info('Handling new input: %s with known task specs: %s', src_path, self.task_specs)
  ...
  def _create_jobs(self, task_spec: TaskSpec, src_path: Optional[str] = None) -> None:
    ...
    for args_and_assignments in product(*inputs):
      ...
      job_spec = JobSpec(job_inputs, job_output, task_spec.path)
