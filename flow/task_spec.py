...
from flow.util import format_timedelta, stringify_bindings
    ...
    def __init__(self, name: str, path_template: PathTemplate) -> None:
        ...
        if name in path_template.placeholders:
            raise InputSpecError(
                'InputSpec <{}> declares same variable identifier that its path template {} declares: "{}"'.format(
                    self, path_template, name
                )
            )
...
class AggregatingInputSpec(InputSpec):
    ...
    def __init__(self, name: str, dictionary: Dict[Tuple[str, ...], str]) -> None:
        ...
        if len(items) != 1:
            raise InputSpecError(
                "AggregatingInputSpec only takes dictionaries with exactly one entry"
            )
        ...
        if invalid_keys:
            raise InputSpecError(
                "AggregatingInputSpec keys {} should be enclosed in curly braces.".format(
                    invalid_keys
                )
            )
        self.locally_bound_variables = [key[1:-1] for key in keys]
        self.path_template = PathTemplate(path_template_string)
        if not set(self.locally_bound_variables).issubset(
            set(self.path_template.placeholders)
        ):
            diff = set(self.locally_bound_variables) - set(
                self.path_template.placeholders
            )
            raise InputSpecError(
                "AggregatingInputSpec declares variables {} which are not bound locally in its path template {}.".format(
                    diff, self.path_template
                )
            )

    def __repr__(self) -> str:
        return "Aggr.In: {}, aggregates {}".format(
            self.path_template, self.locally_bound_variables
        )
...
class DependentInputSpec(InputSpec):
    ...
    def __init__(self, name: str, function: Callable) -> None:
        ...
        if name in self.inputs:
            raise InputSpecError(
                "DependentInputSpec can not depend on variable {} which it is declaring itself.".format(
                    name
                )
            )
...
class TaskSpec(object):
    ...
    def __init__(
        self, inputs: List[InputSpec], output: OutputSpec, src_path: str, name: str
    ) -> None:
    ...
    def to_job_specs(self, initial_bindings: Bindings = {}) -> Iterable[JobSpec]:
    ...
    def all_bindings(self, initial_bindings: Bindings = {}) -> Sequence[Bindings]:
        ...
        for variable_name in sorted_dependencies:
            ...
            for input_spec in input_specs:
                ...
                memoized_values: Dict[FrozenSet[Tuple[str, str]], Set[Value]] = {}
                for bindings in all_bindings:
                    relevant_bs = frozenset(
                        (var, str(value))
    ...
    def manifest(self, all_bindings: Optional[List[Bindings]] = None) -> Dict:
        ...
        return {
            "output": {"template": self.output_spec.path_template.template},
